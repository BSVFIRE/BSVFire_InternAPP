...
export function FG790KontrollView({ 
  ...
  // Styringer på anlegget – status settes her og lagres på anleggsdata_brannalarm
  ...
  const [styringerLukket, setStyringerLukket] = useState(false)
  ...
  async function loadData() {
    ...
    try {
      ...
      // Styringer registrert på anlegget
      const { data: brannalarmData } = await supabase.from('anleggsdata_brannalarm').select('*').eq('anlegg_id', anleggId).maybeSingle()
      if (brannalarmData) {
        ...
        <section className="card !p-0 overflow-hidden" aria-label="Styringer">
          <div className="flex items-center gap-2 px-3 py-2 bg-gray-50 dark:bg-dark-100">
            <button type="button" onClick={() => setStyringerLukket(v => !v)} aria-expanded={!styringerLukket} className="flex items-center gap-2 flex-1 min-w-0 text-left">
              <ChevronRight className={cn('w-4 h-4 text-gray-400 transition-transform', !styringerLukket && 'rotate-90')} />
